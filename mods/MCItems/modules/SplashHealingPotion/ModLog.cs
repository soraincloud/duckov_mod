...
internal static class ModLog
{
    ...
    internal static void Initialize(string? modPath)
    {
        ...
        try
        {
            ...
            {
                var preferred = Path.Combine(modPath, "splashhealingpotion.log");
                if (TrySetLogPath(preferred))
                {
                    WriteRaw($"=== SplashHealingPotion log start {DateTime.Now:yyyy-MM-dd HH:mm:ss} ===\n");
            ...
            var fallbackDir = Path.Combine(Application.persistentDataPath, "SplashHealingPotion");
            var fallback = Path.Combine(fallbackDir, "splashhealingpotion.log");
            ...
            var tmp = Path.Combine(Path.GetTempPath(), "splashhealingpotion.log");
