...
internal static class ModLog
{
    ...
    internal static void Initialize(string? modPath)
    {
        ...
        try
        {
            ...
            {
                var preferred = Path.Combine(modPath, "enderpearl.log");
                if (TrySetLogPath(preferred))
                {
                    WriteRaw($"=== EnderPearl log start {DateTime.Now:yyyy-MM-dd HH:mm:ss} ===\n");
            ...
            var fallbackDir = Path.Combine(Application.persistentDataPath, "EnderPearl");
            var fallback = Path.Combine(fallbackDir, "enderpearl.log");
            ...
            var tmp = Path.Combine(Path.GetTempPath(), "enderpearl.log");
