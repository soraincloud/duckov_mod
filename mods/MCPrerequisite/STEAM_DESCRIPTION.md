...
版本：v1.0.0
更新日期：2026-03-17
更新内容：首个版本，提供 MC 系列 Mod 的统一前置支持。

这是 MC 系列 Mod 的公共前置，用来统一管理工作台与仓库中的 `MC` 分类显示逻辑。启用本前置后，已接入的 MC 系列物品会在工作台和仓库中共用同一套 `MC` 分类入口；未启用前置时，这个分类不会出现，避免每个独立 Mod 各自重复维护同一套分类按钮、图标与标签逻辑。
