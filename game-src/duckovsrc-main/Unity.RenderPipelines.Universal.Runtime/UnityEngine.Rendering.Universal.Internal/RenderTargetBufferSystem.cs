...
internal sealed class RenderTargetBufferSystem
{
	...
	private void ReAllocate(CommandBuffer cmd)
	{
		...
		{
			RenderingUtils.ReAllocateIfNeeded(ref m_A.rtMSAA, in descriptor, m_FilterMode, TextureWrapMode.Clamp, isShadowMap: false, 1, 0f, m_A.name);
		}
		descriptor.msaaSamples = m_B.msaa;
		if (descriptor.msaaSamples > 1)
		{
			RenderingUtils.ReAllocateIfNeeded(ref m_B.rtMSAA, in descriptor, m_FilterMode, TextureWrapMode.Clamp, isShadowMap: false, 1, 0f, m_B.name);
		}
		descriptor.msaaSamples = 1;
		RenderingUtils.ReAllocateIfNeeded(ref m_A.rtResolve, in descriptor, m_FilterMode, TextureWrapMode.Clamp, isShadowMap: false, 1, 0f, m_A.name);
		RenderingUtils.ReAllocateIfNeeded(ref m_B.rtResolve, in descriptor, m_FilterMode, TextureWrapMode.Clamp, isShadowMap: false, 1, 0f, m_B.name);
