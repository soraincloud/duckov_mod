...
public static class ShadowUtils
{
	...
	static ShadowUtils()
	{
		_ShadowBias = Shader.PropertyToID("_ShadowBias");
		_LightDirection = Shader.PropertyToID("_LightDirection");
		_LightPosition = Shader.PropertyToID("_LightPosition");
	...
	private static RenderTextureDescriptor GetTemporaryShadowTextureDescriptor(int width, int height, int bits)
	{
		...
		result.shadowSamplingMode = ((!RenderingUtils.SupportsRenderTextureFormat(RenderTextureFormat.Shadowmap) || SystemInfo.graphicsDeviceType == GraphicsDeviceType.OpenGLES2) ? ShadowSamplingMode.None : ShadowSamplingMode.CompareDepths);
		return result;
	}
	...
	internal static void SetPerLightSoftShadowKeyword(CommandBuffer cmd, bool hasSoftShadows)
	{
		...
		{
			CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT", hasSoftShadows);
		}
	}

	internal static void SetSoftShadowQualityShaderKeywords(CommandBuffer cmd, ref ShadowData shadowData)
	{
		CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT", shadowData.isKeywordSoftShadowsEnabled);
		if (SupportsPerLightSoftShadowQuality())
		{
			CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_LOW", state: false);
			CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_MEDIUM", state: false);
			CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_HIGH", state: false);
			return;
		}
		if (shadowData.isKeywordSoftShadowsEnabled)
		{
			...
			{
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_LOW", state: true);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_MEDIUM", state: false);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_HIGH", state: false);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT", state: false);
				return;
			}
		}
		if (shadowData.isKeywordSoftShadowsEnabled)
		{
			...
			{
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_LOW", state: false);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_MEDIUM", state: true);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_HIGH", state: false);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT", state: false);
				return;
			}
		...
		if (shadowData.isKeywordSoftShadowsEnabled)
		{
			...
			if ((object)asset3 != null && asset3.softShadowQuality == SoftShadowQuality.High)
			{
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_LOW", state: false);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_MEDIUM", state: false);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT_HIGH", state: true);
				CoreUtils.SetKeyword(cmd, "_SHADOWS_SOFT", state: false);
			}
		}
	}
