...
public class InputSystemUIInputModule : BaseInputModule
{
	...
	{
		ref PointerModel pointerStateForIndex = ref GetPointerStateForIndex(index);
	...
	private void OnScrollCallback(InputAction.CallbackContext context)
	{
		...
		if (pointerStateIndexFor != -1)
		{
			...
			pointerStateForIndex.scrollDelta = vector / 1f * scrollDeltaPerTick;
			pointerStateForIndex.eventData.displayIndex = GetDisplayIndexFor(context.control);
		}
	}
	...
	private void FilterPointerStatesByType()
	{
		...
		case UIPointerType.MouseOrPen:
		{
			for (int j = 0; j < m_PointerStates.length; j++)
			{
				ref PointerModel pointerStateForIndex2 = ref GetPointerStateForIndex(j);
				if (m_PointerStates[j].pointerType == UIPointerType.Touch)
				{
					pointerStateForIndex2.leftButton.isPressed = false;
				}
				if (((m_PointerStates[j].pointerType != UIPointerType.MouseOrPen && m_PointerStates[j].pointerType != UIPointerType.Touch) || (m_PointerStates[j].pointerType == UIPointerType.Touch && !pointerStateForIndex2.leftButton.isPressed && !pointerStateForIndex2.leftButton.wasReleasedThisFrame)) && SendPointerExitEventsAndRemovePointer(j))
				{
					j--;
				}
			}
